...
7. Generate one UUID per user request and pass `--task-id <uuid>` and `--intent "<the user's request in one sentence>"` on every command for that request. These are analytics only and do not change behavior.
